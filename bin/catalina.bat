set TITLE="8090"
set CATALINA_BASE=D:\Softs\apache-tomcat-9.0-8090
set CATALINA_HOME=D:\Softs\apache-tomcat-9.0-8090
@echo off
...
%_EXECJAVA% %LOGGING_CONFIG% %LOGGING_MANAGER% %JAVA_OPTS% %JPDA_OPTS% %CATALINA_OPTS% %DEBUG_OPTS% -D%ENDORSED_PROP%="%JAVA_ENDORSED_DIRS%" -classpath "%CLASSPATH%" -Dcatalina.base="%CATALINA_BASE%" -Dcatalina.home="%CATALINA_HOME%" -Djava.io.tmpdir="%CATALINA_TMPDIR%" %MAINCLASS% %CMD_LINE_ARGS% %ACTION% >> %CATALINA_HOME%/logs/catalina.%date:~0,4%.%date:~5,2%.%date:~8,2%.out
goto end
:doSecurityJpda
